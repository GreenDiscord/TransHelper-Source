...
class Economy(commands.Cog):
    ...
    async def shop(self, ctx):
          e = discord.Embed(title="**Shop**", description="Items In The Shop Today:")
          e.add_field(name="**You**", value=f"The price of **You** is 1000000, this changes everyday.")
          e.add_field(name="**Bear**", value=f"The price of **Bear** is 10000, this changes everyday.")
    ...
    async def add_money(self, ctx, ammount, member : discord.Member=None):
          ...
          await self.bot.db.commit()
          e = discord.Embed(title="Added Money", description=f"Added {ammount} into {member} account!")
    ...
    async def remove_money(self, ctx, ammount, member : discord.Member=None):
          ...
          await cursor.execute("UPDATE mail SET balance = balance - ? WHERE user_id=?", (ammount,USER_ID))
          await self.bot.db.commit()
          e = discord.Embed(title="Removed Money", description=f"Removed {ammount} out of {member}'s account!")
          await ctx.reply(embed=e)
        ...
        if item in list1:
          ...
          if item in prices:
            ...
            if result_userBal[0] < itemprice:
              e3 = discord.Embed(title="**Purchase Error Has Happened**", description=f"You need {itemprice} to get {item}!")
              await ctx.send(embed=e3)
            else:
                await cursor.execute("UPDATE mail SET balance = balance - ? WHERE user_id=?", (itemprice,USER_ID))
                await self.bot.db.commit()
                e = discord.Embed(title="**Item Purchased!**", description=f"You Have Purchased {item}!")
                await ctx.send(embed=e)
          else:
                  e2 = discord.Embed(title="**Item Not Purchased!**", description=f"You Have Tried To Purchase {item}, But It Doesn't Exsist!")
                  await ctx.send(embed=e2)
    ...
    async def createaccount(self, ctx):
        ...
        if result_userId == None:
          ...
          await ctx.send(embed=e)
            ...
            e2 = discord.Embed(title="**You already have a account!**", description=f"You already own an account {ctx.author.mention},    'You're not getting any more money from me - {random.choice(lis)}'")
            await ctx.send(embed=e2)
